...
// ═══════════════════════════════════════════════════════════════════
// Exclusion filter — applied to takeoff engine output
// ═══════════════════════════════════════════════════════════════════

export interface ExclusionFilterResult {
  kept: YoloTagInstance[];
  dropped: Array<{ instance: YoloTagInstance; reason: DropReason }>;
}
...
/**
 * Apply exclusion + inclusion rules to a flat list of tag instances.
 *
 * Precedence (critical — tables must beat drawings on overlap):
 *   1. instance center inside any title_block region → drop (inside_title_block)
 *   2. instance center inside any table region      → drop (inside_table)
 *   3. if page has ≥1 drawings region AND center NOT inside any → drop (outside_drawings)
 *   4. otherwise → keep
 *
 * When the page has no `classifiedRegions` (yolo hasn't been loaded, or the
 * classifier found nothing), ALL instances are kept. This is intentional —
 * Map Tags should degrade gracefully; Auto-QTO blocks earlier at preflight.
 */
export function applyExclusionFilter(
  ...
  const kept: YoloTagInstance[] = [];
  const dropped: Array<{ instance: YoloTagInstance; reason: DropReason }> = [];

  for (const inst of instances) {
    const regions = classifiedRegionsByPage[inst.pageNumber];
    if (!regions) {
      kept.push(inst);
      continue;
    }

    const center = bboxCenterMinMax(inst.bbox);

    // 1. Title block exclusion (highest priority)
    if (regions.titleBlocks.some((r) => bboxContainsPoint(r.bbox, center))) {
      dropped.push({ instance: inst, reason: "inside_title_block" });
      continue;
    }

    // 2. Table exclusion (MUST check before drawings inclusion — see precedence rule)
    if (regions.tables.some((r) => bboxContainsPoint(r.bbox, center))) {
      dropped.push({ instance: inst, reason: "inside_table" });
      continue;
    }

    // 3. Drawings inclusion (soft — only applies if drawings regions exist on this page)
    if (regions.drawings.length > 0) {
      const insideDrawings = regions.drawings.some((r) => bboxContainsPoint(r.bbox, center));
      if (!insideDrawings) {
        dropped.push({ instance: inst, reason: "outside_drawings" });
        continue;
      }
    }
